...
const ClothesProduct = () => {
    ...
    const saveUpdate = () => {
        ...
        fetch(`${url}/foodProduct/update/${id}`, {
            method: "POST",
            headers: {
                "Authorization": `Bearer ${token}`,
                'Accept': 'application/json',
    ...
    return (
        <CRow className='d-flex mb-2'>
            ...
            <CCol lg={3} sm={6} className='mb-2'>
                <CButton className="float-end" onClick={() => { setAction(!action), clear() }}>{action ? 'Close' : 'Add New Food Product'}</CButton>
            </CCol>
            {action &&
